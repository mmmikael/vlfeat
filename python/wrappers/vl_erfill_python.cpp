...
PyObject * vl_erfill_python(PyArrayObject & image, double seed)
{
	...
	idx_t i;
	int k, nel, ndims;
	int const dims[] = {image.dimensions[1], image.dimensions[0]};
	val_t const * I_pt;
	int last = 0;
	int last_expanded = 0;
	val_t value = 0;
...
//	}
	...
	/* get dimensions */
	nel = image.dimensions[0] * image.dimensions[1]; //mxGetNumberOfElements(in[IN_I]);
	ndims = image.nd; //mxGetNumberOfDimensions(in[IN_I]);
	//dims = {image.dimensions[1], image.dimensions[0]}; //mxGetDimensions(in[IN_I]);
	I_pt = (val_t const *) image.data; //mxGetData(in[IN_I]);

	/* allocate stuff */
	subs_pt = new int[ndims]; //mxMalloc(sizeof(int) * ndims);
	nsubs_pt = new int[ndims]; //mxMalloc(sizeof(int) * ndims);
	strides_pt = new idx_t[ndims]; //mxMalloc(sizeof(idx_t) * ndims);
	visited_pt = new val_t[nel]; //mxMalloc(sizeof(val_t) * nel);
	members_pt = new idx_t[nel]; //mxMalloc(sizeof(idx_t) * nel);

	er_pt = &seed; //mxGetPr(in[IN_ER]);
...
//		dims[0] = last;
//		out[OUT_MEMBERS]
//				= mxCreateNumericArray(1, dims, mxUINT32_CLASS, mxREAL);
...
//	}
	...
	/* free stuff */
	delete[] members_pt; //mxFree(members_pt);
	delete[] visited_pt; //mxFree(visited_pt);
	delete[] strides_pt; //mxFree(strides_pt);
	delete[] nsubs_pt; //mxFree(nsubs_pt);
	delete[] subs_pt; //mxFree(subs_pt);

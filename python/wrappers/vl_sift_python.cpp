...
/* options */
//uMexOption options[] = { { "Octaves", 1, opt_octaves }, { "Levels", 1,
//		opt_levels }, { "FirstOctave", 1, opt_first_octave }, { "Frames", 1,
//		opt_frames }, { "PeakThresh", 1, opt_peak_thresh }, { "EdgeThresh", 1,
//		opt_edge_thresh }, { "NormThresh", 1, opt_norm_thresh }, { "Magnif", 1,
//		opt_magnif }, { "Orientations", 0, opt_orientations }, { "Verbose", 0,
//		opt_verbose }, { 0, 0, 0 } };
...
PyObject * vl_sift_python(PyArrayObject & image)
{
	...
	data = (vl_sift_pix*) image.data; //(vl_sift_pix*) mxGetData(in[IN_I]);
	M = image.dimensions[1]; //mxGetM(in[IN_I]);
	N = image.dimensions[0]; //mxGetN(in[IN_I]);
	 ...
	 * -------------------------------------------------------------- */
	{
			...
			for (; i < nkeys; ++i) {
				...
				if (nikeys >= 0) {
					vl_sift_keypoint_init(
						filt, &ik, ikeys[4 * i + 1] - 1, ikeys[4 * i + 0] - 1,
						ikeys[4 * i + 2]);
				...
				for (q = 0; q < nangles; ++q) {
					...
					if (reserved < nframes + 1) {
						...
						frames = (double *) realloc(frames, 4 * sizeof(double)
								* reserved);
						//mxRealloc(frames, 4 * sizeof(double)* reserved);
						descr = (vl_uint8 *) realloc(descr, 128
								* sizeof(vl_uint8) * reserved);
						//mxRealloc(descr, 128 * sizeof(vl_uint8) * reserved);
					}
		...
		int dims[2] = {nframes, 4};
		_frames = PyArray_SimpleNewFromData(2, dims, PyArray_DOUBLE, frames);
		assert(_frames);
		PyArray_FLAGS(_frames) |= NPY_OWNDATA;

		dims[1] = 128;
		_descriptors = PyArray_SimpleNewFromData(2, dims, PyArray_UBYTE, descr);
		assert(_descriptors);
